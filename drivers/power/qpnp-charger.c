...
static void qpnp_qc20_check_work(struct work_struct *work)
{
	...
	if (chip->qc20_ta_connected) {
		...
		pr_info("qpnp_qc20_check_work state = %d, qc20_disabled =%d, pre_qc20_disabled =%d,  qc20_ta_connected =%d, voltage = %dV \n"
			, quick_charging_state, chip->qc20_disabled, pre_qc20_disabled, chip->qc20_ta_connected, ret.intval/1000000);
	} else {
		/*                                                             
                                                                     */
...
static void qpnp_qc20_iusb_work(struct work_struct *work)
{
	...
	if (chip->qc20_ta_connected) {
		...
		pr_info("qpnp_qc20_iusb_work iusb = %d \n", chip->max_usb_qc20_current);
