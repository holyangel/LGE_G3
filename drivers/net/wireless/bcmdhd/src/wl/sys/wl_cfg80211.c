 ...
 *
 * $Id: wl_cfg80211.c 459929 2014-03-05 12:29:56Z $
 */
/* */
#include <typedefs.h>
#include <linuxver.h>
	...
	struct vif_params *params)
{
	...
	if (unlikely(!wl_get_drv_status(wl, READY, _ndev))) {
...
static s32 wl_update_bss_info(struct wl_priv *wl, struct net_device *ndev, bool roam)
{
	...
	*(u32 *) wl->extra_buf = htod32(WL_EXTRA_BUF_MAX);
	...
	wl_update_prof(wl, ndev, NULL, &channel, WL_PROF_CHAN);
...
#if (LINUX_VERSION_CODE > KERNEL_VERSION(2, 6, 39)) || defined(WL_COMPAT_WIRELESS)
	/*                                                           */
	channel = (u32 *)wl_read_prof(wl, ndev, WL_PROF_CHAN);
	if (*channel <= CH_MAX_2G_CHANNEL)
		band = wiphy->bands[IEEE80211_BAND_2GHZ];
	else
		band = wiphy->bands[IEEE80211_BAND_5GHZ];
...
wl_stop_wait_next_action_frame(struct wl_priv *wl)
{
	if (wl_get_drv_status_all(wl, FINDING_COMMON_CHANNEL)) {
		...
		if (wl->afx_hdl != NULL) {
			if (wl->afx_hdl->dev != NULL) {
				wl_clr_drv_status(wl, SCANNING, wl->afx_hdl->dev);
				wl_clr_drv_status(wl, FINDING_COMMON_CHANNEL, wl->afx_hdl->dev);
			}
			wl->afx_hdl->peer_chan = WL_INVALID;
		}
		complete(&wl->act_frm_scan);
		WL_DBG(("*** Wake UP ** Working afx searching is cleared\n"));
	} else if (wl_get_drv_status_all(wl, SENDING_ACT_FRM)) {
...
static s32 __wl_cfg80211_down(struct wl_priv *wl)
{
	...
	wl->wl11u = FALSE;
	wl->iw_ie_len = FALSE;
	memset(wl->iw_ie, 0, IW_IES_MAX_BUF_LEN);
#endif
